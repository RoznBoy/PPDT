...
def enc_and_save(enc, df, n, d, ndata, logN, ctxt_path, context, keypack):
    ...
    for cname in df.columns:
        if ('label' in cname):
            ...
            m = [0]*(1<<(logN-1))
            # print('check point 1', len(m), (1<<(logN-1)))
            for nd in range(n*d):
                for size in range(df[cname].size):
                    index = nd*pow_2_ndata+size
                    # print('check point 2', index, nd, size)
                    m[index]=df[cname][size]
            msg.set_data(data=m)
            enc.encrypt(msg,keypack,label_ctxt)
            label_ctxt.to_device()
            label_ctxt.save(ctxt_path+cname+".ctxt")
        
        if ('X' in cname):
            for size in range(df[cname].size):
                index = num*pow_2_ndata+size
                mx[index]=df[cname][size]
            num+=1
    
    x_msg.set_data(data=mx)        
    c = heaan.Ciphertext(context)
    enc.encrypt(x_msg,keypack,c)
    c.to_device()
    c.save(ctxt_path+"X.ctxt")

def encrypt_and_save_big(df, ctxt_path, n, d, t, ndata, logN, context, enc, pk):
    N = _get_smallest_pow_2(ndata)
    d1 = (1<<(logN-1))//N
    ctxt_num = int(np.ceil(d/d1))
    for k in range(1,n+1):
        for i in range(ctxt_num):
            x_input = []
            #li = []
            if d1 < d: 
                for j in range(1,d1+1):
                    X = 'X'+str((i+1)+ctxt_num*(j-1))+'_'+str(k)
                    new_x = df[X].to_list() + [0]*(N - ndata)
                ...
                for j in range(1,d+1):
                    ...
                    new_x = df[X].to_list() + [0]*(N - ndata)
                    x_input = x_input + new_x
                x_input = x_input + [0]*((1<<(logN-1))-N*d)
            input_m = heaan.Message(logN-1)
            input_m.set_data(x_input)
            input_ctxt = heaan.Ciphertext(context)
            enc.encrypt(input_m,pk,input_ctxt)
            #print(f"X{i+1}_{k}.ctxt")
            input_ctxt.save(ctxt_path + f"X{i+1}_{k}.ctxt")


    for yname in df.columns[0:t]:
        new_y = df[yname].to_list() + [0]*(N - ndata)
        input_y = new_y * d1
        input_m = heaan.Message(logN-1)
        input_m.set_data(input_y)
        y_ctxt = heaan.Ciphertext(context)
        enc.encrypt(input_m, pk, y_ctxt)
        y_ctxt.save(ctxt_path + f"{yname}" + ".ctxt")
        
    input_ctxt.to_host()
    y_ctxt.to_host()
    del input_ctxt, y_ctxt
