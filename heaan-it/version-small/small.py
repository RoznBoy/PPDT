...
m = [1]*(num_gini) + [0]*(num_slot-num_gini)
...
if n_slot*len(_node) <= num_slot:
    ...
    for i in range(len(_node)):
        ...
        start = time.time()
        gini = mod.find2Gini(out_cdict,meta['d'],meta['n'],meta['t'],meta['ndata'])
        print(L.id, 'find2Gini time: %.5f sec' %(time.time()-start))

        gini = gini.__rshift__(n_slot*i)
        gini_total = gini_total + gini
    ...
    # print('total gini: ')
    # mod.print_ctxt(gini_total,dec,sk,logN,n_slot*len(_node))
    # print('total y: ')
    # mod.print_ctxt(y_total,dec,sk,logN,nslot*len(_node))
    
    start = time.time()
    cmin_sum = mod.findMinPosMany(gini_total,meta['d'],meta['n'],num_gini,n_slot,len(_node))
    print(len(_node), 'findMinPosMany time: %.5f sec' %(time.time()-start))
    start = time.time()
    ...
    for i in range(len(_node)):
        ...
        cmin = cmin_sum.__lshift__(n_slot*i)
        cmin = m_gini * cmin
        # print('DT_Learn 시작 전 cmin: ')
        # mod.print_ctxt(cmin,dec,sk,logN,n_slot)
        ...
        start = time.time() 
        Lca,Lcv,Lcy,Lby = mod.DT_Learn(L,out_cdict,cmin,cy,meta['d'],meta['n'],meta['t'],meta['ndata'],model_path)
        print(L.id, 'DT_Learn time: %.5f sec' %(time.time()-start))
        
        start = time.time() 
        Lca = Lca.__rshift__(interval*(i%one_ctxt_node))
        save_ca = Lca + save_ca
        Lcv = Lcv.__rshift__(interval*(i%one_ctxt_node))
        save_cv = Lcv + save_cv
        ...
        if cnt == one_ctxt_node:
            ca_list.append(save_ca)
            cv_list.append(save_cv)
    ...
    start = time.time()
    mod.preCom(save_path,model_path,node_key,ca_list,cv_list,cy_list,by_list,num_list,interval,parent_ctxt_num,meta['d'],meta['n'],meta['t'],depth)
    print(len(_node), 'evaluation 하기 위한 암호문 저장 시간: %.5f sec' %(time.time()-start))
...
####################################################################################    
    ...
    for k in range(int(np.ceil(len(_node)/max_min4))):
        ...
        print(len(_node), 'findMaxPosMany time: %.5f sec' %(time.time()-start))
        # print('cmin sum: ')
        # mod.print_ctxt(cmin_sum,dec,sk,logN,n_slot)
        # print('cy sum: ')
        # mod.print_ctxt(cy_sum,dec,sk,logN,nslot)
        ...
        for i in range(len(small_node)):
            ...
            start = time.time()
            Lca = Lca.__rshift__(interval*int((max_min4*k+i)%one_ctxt_node))
            save_ca = Lca + save_ca
            Lcv = Lcv.__rshift__(interval*int((max_min4*k+i)%one_ctxt_node))
            save_cv = Lcv + save_cv
